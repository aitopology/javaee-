...
public class StudentDaoImpl implements StudentDao {
    ...
    public boolean existsByNo(String no) {
        ...
        Integer count = this.jdbcTemplate.queryForObject(sql, Integer.class, no);
    ...
    public Student queryByNo(String no) {
        ...
        List<Student> students = this.jdbcTemplate.query(sql, new RowMapperImpl(), no);
        if (students.isEmpty()) return null;
        if (students.size() > 1) {
            throw new IncorrectResultSizeDataAccessException(1, students.size());
        }
    ...
    public int updateByNo(Student student) {
        ...
        System.out.println("Parameters: name=" + student.getName() + ", age=" + student.getAge() + ", no=" + student.getNo());
